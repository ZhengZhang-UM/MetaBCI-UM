...
class PCA_classifier:
    ...
    def updata_model(self):
        ...
        for k in range(self.n_components):
            ...
            distances = self._calculate_mahalanobis_distances(_buf, k)
            self.determine_outclass_threshold(distances, k)
            self.determine_inclass_threshold(distances, k)
    ...
    def _calculate_mahalanobis_distances(self, samples, class_idx):
        """计算样本到类中心的马氏距离"""
        center = self.means_[class_idx]
    ...
    def predict(self, x):
        ...
        for i, sample in enumerate(x):
            for k in range(self.n_components):
                # 使用马氏距离替代欧氏距离
                diff = sample - self.means_[k]
                dists[i, k] = np.sqrt(np.dot(np.dot(diff, self.inv_covariances_[k]), diff.T))
