...
from .utils import NeuroScanPort, NeuraclePort, _check_array_like, Light_trigger, Virtual_trigger
import threading
...
class Emotion:
    ...
    def __init__(self, win, trigger_interval=5, refresh_rate=60, **kwargs): #colorSpace="rgb", allowGUI=True,
        ...
        # ===== 新增：文本刺激预初始化 & 全局参数 =====
        self.text_stims = {}  # 预存文本刺激对象，避免重复创建
        self.rating_scale_range = kwargs.get("rating_scale_range", (0, 10))  # 全局评分范围
        # 预加载所有文本类实验的刺激对象
        for block_name, params in self.experiment_params.items():
            if self.experiment_setup.get(block_name) == "text_rating":
                self.text_stims[block_name] = visual.TextStim(
                    win=self.win,
                    text=params.get("text", ""),
                    font="Microsoft YaHei",  # 支持中文，避免乱码
                    color=[1, 1, 1],
                    units="pix",
                    wrapWidth=self.win.size[0] * 0.8,  # 自动换行
                    height=36,
                    alignHoriz='center',
                    alignVert='center'
                )
    ...
    def text_rating(self, block_name, trigger_label, stim_params):
        """文本情绪评分范式（封装原stim_demo的核心逻辑）
        Parameters:
            block_name: 实验块名称
            trigger_label: 当前块的trigger编码
            stim_params: 该块的配置参数，来自experiment_Stimulus
        """
        # 读取配置参数，带默认值
        display_duration = stim_params.get("display_duration", 0)  # 0表示等待按键
        pre_rest = stim_params.get("pre_stimulus_rest", 1.0)
        post_rest = stim_params.get("post_stimulus_rest", 0.5)
        rating_type = stim_params.get("rating", "VA")
        # 确定评分维度
        if isinstance(rating_type, list):
            rating_dims = rating_type
        elif rating_type in self.DEFAULT_RATINGS:
            rating_dims = self.DEFAULT_RATINGS[rating_type]
        else:
            rating_dims = self.DEFAULT_RATINGS["VA"]  # 默认VA维度

        # 获取预初始化的文本刺激对象
        text_stim = self.text_stims.get(block_name)
        if not text_stim:
            # 兜底：如果未预初始化则临时创建
            text_stim = visual.TextStim(
                win=self.win,
                text=stim_params.get("text", ""),
        ...
        # 2. 发送trigger并显示文本
        if self.port:
            self.port.setData(trigger_label)
        start_time = time.time()
        if display_duration > 0:
            # 固定时长显示，期间周期性发trigger
            while (time.time() - start_time) < display_duration:
                text_stim.draw()
                self.win.flip()
                if 'q' in event.getKeys():
                    core.quit()
                # 按interval周期性发trigger
                if self.port and (time.time() - start_time) % self.trigger_interval < 0.05:
                    self.port.setData(trigger_label)
        else:
            # 等待按键继续
            text_stim.draw()
        ...
        # 5. 评分流程
        scores = self.show_rating_window(rating_dims, title=f"{block_name} 评分")
        # 6. 保存结果
        self.save_scores(block_name, scores, additional_info={
            "trigger": trigger_label,
            "display_duration": display_duration if display_duration>0 else "manual"
        })
...
def paradigm(
    ...
    if pdim == "ssvep":
        ...
        VSObject.ask_ready("情绪实验范式")

        # 1. 预校验所有实验方法，避免运行时报错

        for name in VSObject.block_name:

            method_name = VSObject.experiment_setup[name]

            if not hasattr(VSObject, method_name) or not callable(getattr(VSObject, method_name)):
                raise AttributeError(f"未知实验方法: {method_name}，请检查experiment_setup配置")

        # 2. 启动实时反馈（若开启）

        if inlet and hasattr(VSObject, 'start_live_data'):
            VSObject.start_live_data(inlet)

        # 3. 按顺序执行所有实验块

        for block_idx, name in enumerate(VSObject.block_name):

            method_name = VSObject.experiment_setup[name]

            method = getattr(VSObject, method_name)

            trigger_label = VSObject.trigger_labels[name]

            stim_params = VSObject.experiment_params.get(name, {})

            # 实验前提示

            VSObject.show_message(

                f"即将开始第 {block_idx + 1}/{len(VSObject.block_name)} 个任务：{name}",
            ...
            method(name, trigger_label, stim_params)  # 调用对应实验方法
            ...
            if block_idx != len(VSObject.block_name) - 1:
                VSObject.show_message("休息一下，按空格继续下一个任务", wait_for_space=True)

        # 4. 实验结束清理

        VSObject.show_message("本轮情绪实验全部结束，感谢参与！", wait_for_space=False, duration=3.0)

        if inlet and hasattr(VSObject, 'stop_live_data'):
